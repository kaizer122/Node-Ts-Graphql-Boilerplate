...
export default {
  Mutation: {
    ...
    checkResetCode: async (_, { email, code }) => {
      try {
        let user: IUserModel = null;
        user = await PlayerModel.findOne({ email: email.toLowerCase() });
        if (!user) {
        ...
        if (user.resetPasswordCode !== code) return new UserError("Code invalide.");
    ...
    resetPassword: async (_, { email, password, token }) => {
      try {
        let user: IUserModel = null;
        user = await PlayerModel.findOne({ email: email.toLowerCase() });
        if (!user) {
        ...
        if (user.resetPasswordToken !== token) return new UserError("Votre token de réinisialisation est invalide.");
        if (user.resetPasswordExpires < Date.now()) return new UserError("Votre token de réinisialisation a expiré.");
